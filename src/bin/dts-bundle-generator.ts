...
import { normalizeBundlerConfig } from '../config-file/normalize-config';
...
interface ParsedArgs extends yargs.Arguments {
	...
	'preserve-triple-slash-references': boolean;
...
function parseArgs(): ParsedArgs {
	return yargs
		...
		.option('preserve-triple-slash-references', {
			type: 'boolean',
			default: true,
			description: 'Allows auto preserve from entry files to the output',
		})
		.version()
		.strict()
...
function main(): void {
			...
			entries: args._.map<ConfigEntryPoint>((entryPath: string | number) => {
				return {
					...
					output: {
						...
						preserveTripleSlashReferences: args['preserve-triple-slash-references'],
					},
	...
	bundlerConfig = normalizeBundlerConfig(bundlerConfig);
